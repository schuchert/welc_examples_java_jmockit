...
public class DealPathFinder {
   ...
   LinkedList<LinkedList<Conversion>> findDealsOfLengthLessThan(
         int maxPathLength) {
      Map<String, String> currencySymbols = CurrencyConversion
            .currencySymbols();
      List<Conversion> allPairs = allPairs(baseCurrency,
            currencySymbols.keySet());
      ...
      for (String current : allCurrenciesWithConversion) {
         ...
         allPairs.addAll(new DealPathFinder().startingWith(current).allPairs(
               current, allMinusCurrent));
      }
   ...
   List<Conversion> allPairs(String baseCurrency, Collection<String> set) {
